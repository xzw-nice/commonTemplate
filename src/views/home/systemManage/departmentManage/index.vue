<template>
	<div class="templatePage-layout">
		...
		<div v-loading="loading" class="templatePage-layout-table">
			...
			>
				...
				<el-table-column align="center" fixed="right" label="操作" width="180">
					<template #default="{ row }">
						<el-button v-has="'sys:dept:update'" icon="Edit" link type="primary" @click="editBtn(row)"
							>编辑
						</el-button>
						<el-button v-has="'sys:dept:del'" icon="Delete" link type="danger" @click="delBtn(row)"
							>删除
						</el-button>
					</template>
				</el-table-column>
		...
		<save-dialog
			...
			:depart-tree-list="useTree.tableData.value"
			@getList="getList"
		/>
	</div>
</template>
...
import useTreeTable from "@/utils/hooks/useTreeTable.js";

const loading = ref(false)

const useTree = useTreeTable()
...
function getList() {
  getDepartmentList({
      ...
      useTree.bindData(
          res?.records?.length?res.records:[],
          "pid",
          "id"
      )
...
// 分页
function pageList() {
  useTree.tableFilter({ page: searchList.current, size: searchList.size,filter:{name:searchList.name} })
}
