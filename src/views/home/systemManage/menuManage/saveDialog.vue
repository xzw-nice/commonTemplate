<template>
	...
	>
		...
		>
			...
			<el-form-item label="权限字符：" prop="authStr" required>
				<el-input
...
const formRules = {
	...
	path: [{ required: true, message: "请输入菜单路径/地址", trigger: "blur" }],
	icon: [{ required: true, message: "请选择菜单图标", trigger: "change" }],
	authStr: [{ required: true, message: "请输入权限字符", trigger: "blur" }]
}
	...
	(val) => {
		if (val.id) {
			...
			if (val.pid) {
				...
				menuForm.value.pid = str ? str.split(",") : []
			}
		} else {
			menuForm.value = {
				pid: null,
