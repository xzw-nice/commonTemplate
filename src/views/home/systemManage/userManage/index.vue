<template>
	<div class="templatePage-layout">
		...
		<div v-loading="loading" class="templatePage-layout-table">
            ...
            <el-button v-has="'sys:user:rest:password'" icon="Lock" link type="warning" @click="resetPsd(row)"
            >重置密码
            </el-button>
					</template>
				</el-table-column>
    ...
    type: "warning"
      ...
      .then(() => {
        resetCipher({
          ...
          if(e.id === useUser.user.info.id){
            ElMessage.success("当前用户密码已重置，请重新登录")
            // 退出重新登录
            setTimeout(()=>{
              router.push('/login')
            },500)
