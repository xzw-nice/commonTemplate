 ...
 * @param targetList 目标树
 * @param  value 目标值
 * @param  key 值对应的属性名
 * @param  parentValue 当前级的往前推的所有父级字符串拼接
...
export function getTreeLabelByValue(
	...
	let label: string = ""
	if (targetTree.length) {
		for (let i: number = 0; i < targetTree.length; i++) {
			const current: Record<string, any> = targetTree[i]
			if (current[valueKey] === value) {
				label = current[labelKey]
				break
			} else {
				if (current[childrenKey]?.length) {
					label = getTreeLabelByValue(
						current[childrenKey],
						value,
						valueKey,
						labelKey,
 ...
 * @return 颜色数组
 */
export function generateUniqueColors(length: number): string[] {
 ...
 * @return 颜色数组
 */
export function generateLightColors(length: number): string[] {
	const colors: string[] = []
	const baseValue = 200 // 基础亮度值，可以根据需要调整
	const step: number = Math.floor(55 / length) // 每个颜色之间的亮度差异

	for (let i: number = 0; i < length; i++) {
		// 生成RGB值，确保它们在浅色范围内
		const r: number = baseValue + i * step
		const g: number = baseValue + (((i + 1) * step) % 256) // 稍微改变G值以避免完全重复
		const b: number = baseValue + (((i + 2) * step) % 256) // 稍微改变B值

		// 将RGB值转换为十六进制颜色代码
		const color: string = `#${r.toString(16).padStart(2, "0")}${g
			.toString(16)
 ...
 * @param value 变量值
 */
export function checkVarType(value: any) {
